...
type HttpResponseMessageExtensions () =
  ...
  [<Extension>]
  static member FormatAsString (res : HttpResponseMessage) = sprintf "%A %A -> %A" res.RequestMessage.Method res.RequestMessage.RequestUri res.StatusCode 
...
type Vulnerability =
  ...
  static member Info desc response payloads =
    Vulnerability.fromRespContent response desc Info payloads
  /// Creates a vulnerability with a Medium severity.
  static member Medium desc response payloads =
    Vulnerability.fromRespContent response desc Medium payloads
  /// Creates a vulnerability with a High severity.
  static member High desc response payloads =
    Vulnerability.fromRespContent response desc High payloads
  /// Creates a vulnerability (Medium) with a short XSS description.
  static member Xss response payloads =
  ...
  static member Sql response payloads =
  ...
  static member Dos response payloads =
...
module Vuln =
  ...
  /// Creates a vulnerability with a Info severity.
  let info desc response payloads = Vulnerability.Info desc response payloads
  /// Creates a vulnerability with a Medium severity.
  let medium desc response payloads = Vulnerability.Medium desc response payloads
  /// Creates a vulnerability with a High severity.
  let high desc response payloads = Vulnerability.High desc response payloads
  /// Creates a vulnerability (Medium) with a short XSS description.
  let xss response payloads = Vulnerability.Xss response payloads
  /// Creates a vulnerability (High) with a short SQL description.
  let sql response payloads = Vulnerability.Sql response payloads
  /// Creates a vulnerability (Info) with a short rate-limit description.
  let dos response payloads = Vulnerability.Dos response payloads
  /// Creates a vulnerability (High) with a short privilege escalation description.
  let privilEscal response payloads = Vulnerability.PrivilegeEscalation response payloads
...
type Response = 
  Response of HttpResponseMessage with
    interface IDisposable with
      member this.Dispose () =
        match this with Response res -> res.Dispose ()
...
type RequestExtensions () =
  /// Adds routing paths to the request.
  [<Extension>]
  static member Routes (req, [<ParamArray>] paths : string array) = Req.routes (List.ofArray paths) req
  /// Adds a single route path to the request.
  [<Extension>]
  static member Route (req, path) = Req.route path req
  /// Adds query string parameters to the request.
  [<Extension>]
  static member Parameters (req, [<ParamArray>] parameters : (string * string) array) = Req.parameters (List.ofArray parameters) req
  /// Adds a single query string parameter to the request.
  [<Extension>]
  static member Parameter (req, name, value) = Req.parameter name value req
  /// Adds HTTP headers to the request.
  [<Extension>]
  static member Headers (req, [<ParamArray>] headers : (string * string) array) = Req.headers (List.ofArray headers) req
  /// Adds a single HTTP header to the request.
  [<Extension>]
  static member Header (req, name, value) = Req.header name value req
  /// Sets the HTTPS client certificate to the request.
  [<Extension>]
  static member Certificate (req, certificate) = Req.certificate certificate req
  /// Sets the HTTP method for the request.
  [<Extension>]
  static member Method (req, method) = Req.method method req
  /// Sets the request body with a content type.
  [<Extension>]
  static member Body (req, content, contentType) = Req.body content contentType req
  /// Replace all matching occurrences in the request content with a value.
  [<Extension>]
  static member BodyReplaceRegex (req, pattern, value) = Req.body_regex pattern value req
  /// Replaces all text between a specified indicator (ex. '#to-be-replaced#') with a value.
  /// Remark that this uses a regular expressions so valid regex charaters should be escaped.
  [<Extension>]
  static member BodyReplaceBetween (req, varIndicator, value) = Req.body_between varIndicator value req
  /// Replaces the content type of the request body.
  [<Extension>]
  static member ContentType (req, contentType) = Req.contentType contentType req
...
module Api =
  ...
  let private sendRequest payloads options = async {
    let query = String.Join ("&", List.map (fun (h, v) -> sprintf "%s=%s" h v) options.Params)
    let query = if String.IsNullOrWhiteSpace query then query else "?" + query
    let route = String.Join ("/", options.Routes)
    let url = sprintf "%s/%s%s" (options.BaseUrl.TrimEnd '/') route query

    use handler = new System.Net.Http.HttpClientHandler ()
    Option.iter (fun cert -> handler.ClientCertificates.Add cert |> ignore) options.ClientCertificate
    use client = new HttpClient (handler)

    use req = new HttpRequestMessage (options.Method, url)
    for (headerName, headerValue) in options.Headers do
        req.Headers.Add (headerName, headerValue)

    Option.iter (fun b -> 
      let content = List.fold (fun acc f -> f acc) b.Content b.Replacements
      req.Content <- new StringContent(content, Encoding.UTF8, b.ContentType)) options.Body

    try let! response = client.SendAsync (req) |> Async.AwaitTask
        return payloads, Response response
    with ex ->
        let res = new HttpResponseMessage(Net.HttpStatusCode.InternalServerError)
        res.Content <- new StringContent (ex.Message)
        res.RequestMessage <- req
        return payloads, Response res }
  ...
  let scan (req : Request) scan = async {
      let! responsesForPayloads = 
        injectRndPayloadsInReq scan req
        |> fun xs -> Seq.map (fun (ps, r) -> sendRequest ps r) xs
        |> Async.ParallelWithThrottle req.MaxConcurrentRequests
      try
        let! vulnerabilities =
...
module Res =
  ...
  let contains expected (Response response) = async {
    let! content = response.Content.readAsStringAsync()
    return content.Contains (expected) }
  /// Determines if a response has one of the allowed status codes.
  let allowed statusCodes (Response response) = Seq.contains response.StatusCode statusCodes
  /// Determines if a response header exists in the response.
  let exists headerName (Response response) = response.Headers.Contains(headerName)
  /// Gets the response headers in a Map<_, _> type.
  let headers (Response response) : Map<string, string seq> = 
...
type ResponseExtensions () =
  ...
  /// and creates a vulnerability for it when it does.
  [<Extension>]
  static member Contains (response, expected) =
    if isNull expected then nullArg "expected"
    Res.contains expected (Response response)
  /// Determines if a response has one of the allowed status codes.
  [<Extension>]
  static member Allowed (response, statusCodes) =
    if isNull statusCodes then nullArg "statusCodes"
    Res.allowed statusCodes (Response response)
  /// Determines if a response headers exists in the response.
  [<Extension>]
  static member Exists (Response response, headerName) =
    if isNull headerName then nullArg "headerName"
    Res.exists headerName (Response response)
